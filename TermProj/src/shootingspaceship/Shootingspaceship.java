...
public class Shootingspaceship extends JPanel implements Runnable {
    ...
    private Image[] backgrounds = {
            new ImageIcon(getClass().getResource("/img/desert.png")).getImage(),
    ...
    public Shootingspaceship(AppFrame frame, int selectedShipType, int selectedMissileType) {
        ...
        // 선택된 스테이지 번호 불러오기
        selectedStage = frame.getSelectedStage();
        ...
        stages = new Stage[] {
        	    new Stage(1, 2, 10, false, 0, 0),        // 0: 사막
        ...
        spawnInitialEnemies(); // 적 몬스터, 보스 등장
    ...
    private void spawnInitialEnemies() {
        ...
        // 일반 적 생성(랜덤x, y=0)
        for (int i = 0; i < currentStage.getMaxEnemies(); i++) {
            enemies.add(
                new Enemy(
                    new Random().nextInt(width-20) + 10, // X좌표
                    0,									// Y (맨위)
                    0, currentStage.getEnemySpeed(),	// xSpeed=0, ySpeed=스테이지별
                    width, height, 0,					// 화면 경계, ySpeed 증가 값
                    currentStage.getEnemyHp()			// 체력
                )
            );
        }
    ...
    private class ShipControl implements KeyListener {
        public void keyPressed(KeyEvent e) {
            switch (e.getKeyCode()) {
                ...
                case KeyEvent.VK_UP:
                	// 미사일 아이템 구분 (싱글,더블 샷)
                    String itemName = parentFrame.getSelectedItem() != null ? parentFrame.getSelectedItem().getName() : "item_defaultMissle";
        ...
        while (true) {
            ...
            for (int i = 0; i < shots.length; i++) {
                if (shots[i] != null) {
                    ...
                    if (shots[i].getY() < 0) shots[i] = null;
                }
            }
            if (playerMoveLeft) player.moveX(playerLeftSpeed);
            else if (playerMoveRight) player.moveX(playerRightSpeed);
            ...
            for (Iterator<EnemyBullet> it = enemyBullets.iterator(); it.hasNext();) {
                ...
                b.move();
                if (b.checkCollision(player) || !b.isAlive()) it.remove();
            }
            // 적-총알, 적-플레이어 충돌
    ...
    public void paintComponent(Graphics g) {
        ...
        for (EnemyBullet b : enemyBullets) b.draw(g);
